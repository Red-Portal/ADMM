...
admm_enet(MatrixXf x_, VectorXf y_,
          ...
          double const alpha_, ADMMParam const& opts_)
{
    const int n = x_.rows();
    const int p = y_.cols();
    ...
    if(n > p)
        solver_tall = new ADMMEnetTall(x_, y_, alpha_, eps_abs, eps_rel);
    else
        solver_wide = new ADMMEnetWide(x_, y_, alpha_, eps_abs, eps_rel);
    ...
    if(nlambda < 1)
    {
        ...
        lambda_.setLinSpaced(nlambda, std::log(lmax), std::log(lmin));
        lambda_ = lambda_.exp();
